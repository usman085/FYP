...
const AppointmentDataTable = () => {
        ...
        return ( <
                ...
                <
                th className = "text-secondary text-left"
                scope = "col" > Sr No < /th> <
                ...
                tbody > {
                    ContextData.allBookedAppointments.map(ap =>
                        ...
                        <
                        select onClick = {
                            () => setSelectAppointment(ap)
                        }
                        onChange = { e => handleStatusChange(e.target.value) }
                        className = { ap.status == "Rejected" ? "btn btn-danger" : ap.status == "Approved" ? "btn btn-success" : "btn btn-info" } >
                        <
                        option selected = { ap.status == "Pending" }
                        className = "bg-white text-secondary" > Pending < /option> <
                        option selected = { ap.status == "Approved" }
                        className = "bg-white text-secondary" > Approved < /option> <
                        option selected = { ap.status == "Rejected" }
                        className = "bg-white text-secondary" > Rejected < /option> < /
                        select >


                        <
                        button onClick = {
                            () => openDataEditModal(ap._id)
                        }
                        className = "btn ml-1 btn-warning text-white" > < FontAwesomeIcon icon = { faPencilAlt }
                        /> </button >
                        <
                        /td> < /
                        tr >
...

                {
                    ...
                    <
                    h5 className = "text-primary text-center mb-5" > { selectAppointment.patientInfo.name }
                    's Appointment</h5> <
                    div className = "form-group row" >
                    <
                    label htmlFor = ""
                    className = "col-4" > Date < /label> <
                    input type = "text"
